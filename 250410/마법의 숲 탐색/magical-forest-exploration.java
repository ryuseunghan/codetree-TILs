...
//        for(int i = 4; i <= r; i++){
//            for(int j = 1; j <= c; j++){
//                if(graph[i][j]) System.out.print("T ");
...
//        }
    ...
    static int fairyMove(Golem golem){
        ...
        while(!queue.isEmpty()){
            ...
            for(int i = 0; i < 4; i++){
                ...
                // 그래프 내부이고, 방문하지 않고, 이동 가능한 골렘일 때 이동
                if(canMove(newNode) && !visited[newNode[0]][newNode[1]] && graph[newNode[0]][newNode[1]]){
                    // 아래로만 내려가기
                    if(newNode[0] >= node[0]){
    ...
    static int checkAndMove(Golem golem){
        ...
        // 남쪽 가능 - 오 아래 왼 비교
        if(point_low[0] + 1 <= r && !graph[point_low[0] + 1][point_low[1]] && !graph[point_right[0] + 1][point_right[1]] && !graph[point_left[0] + 1][point_left[1]]){
            golem.move(1, 0, 0);
            return 1;
        }
        // 서쪽 회전 - 상 좌 하 비교
        if(point_left[1] - 1 > 0 && !graph[point_high[0]][point_high[1] - 1] && !graph[point_left[0]][point_left[1] - 1] && !graph[point_low[0]][point_low[1] - 1]){
            // 좌 하 비교
            if(point_low[0] + 1 <= r && !graph[point_left[0]+1][point_left[1] - 1] && !graph[point_low[0]+1][point_low[1] - 1]){
                golem.move(1, -1, -1);
                return 2;
            }
        }
        // 동쪽 회전 - 상 우 하 비교
        if(point_right[1] + 1 <= c && !graph[point_high[0]][point_high[1] + 1] && !graph[point_right[0]][point_right[1] + 1] && !graph[point_low[0]][point_low[1] + 1]){
            // 우 하 비교
            if(point_low[0] + 1 <= r && !graph[point_right[0]+1][point_right[1] + 1] && !graph[point_low[0] + 1][point_low[1] + 1]){
                golem.move(1, 1, 1);
                return 2;
            }
        }
        ...
        // 그래프에 골렘 표시
        graph[golem.row][golem.col] = true;
        graph[golem.row+ 1][golem.col] = true;
        graph[golem.row][golem.col + 1] = true;
        graph[golem.row - 1][golem.col] = true;
        graph[golem.row][golem.col - 1] = true;
        return 4;
    }
